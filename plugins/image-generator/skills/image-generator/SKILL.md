...
description: Looma 图片生成技能 ImageGenerator。当用户要求生成、绘制、出图、改图、重绘、图生图、局部编辑、下载或保存图片，或提到 ImageGenerator、图片生成、AI出图、生图、海报、封面、公众号配图、小红书封面、视频号封面、抖音封面、知识图谱、信息图、gpt-image-2、DALL·E 风格 /v1/images/generations 请求时使用。生成成功后必须把图片主动发送到当前会话，而不是只报告本地路径。上游可能排队数分钟，须耐心用同一 task_id 间隔轮询，禁止因慢而重提。
---
...
生成成功后（`task_status=SUCCESS` 且 `absolute_image_paths` / `image_paths` 非空），**禁止**只回复“已保存到某某路径”。

必须让用户在当前会话里看到图片：

1. **优先（Looma）**：宿主会在 skill 脚本成功且路径有效时 **自动把图片挂为会话附件并渲染预览**。正文只需简短确认「已生成并展示」，**不要**再写会裂图的本地 Markdown 图片（如 `![](C:\...)` / `![](E:/...)`），宿主已展示附件即可。
2. **仅当**宿主未挂载附件时，再用 Markdown（优先 `~/.looma/assets/attachments/` 下的托管路径，正斜杠）：

```markdown
![生成结果](C:/Users/you/.looma/assets/attachments/attachment_xxx.png)
```

3. 仅当路径无效或文件不存在时，才说明失败原因；不要假装已展示。
...
上游服务**不稳定且可能很慢**：有时约 30 秒出图，有时需要 **数分钟**。`NOT_START` / `IN_PROGRESS` **不等于失败**。

1. **只提交一次**：同一用户需求只 `--no-wait` 提交一次，拿到 `task_id`。
2. **只用同一 task_id 查询**：之后所有轮询必须是：

```bash
python scripts/generate_image.py --task-id "<同一个 task_id>"
```

3. **禁止因慢而重提**：在未收到 `FAILURE`、未收到用户明确“重试/换尺寸重做”之前，**禁止**再次 `--prompt` 提交新任务（不要换 size/quality 开第二个任务）。
4. **接受长时间排队**：连续多次 `NOT_START` 仍属正常；继续同一 `task_id` 查询即可。Looma 会在轮询之间自动间隔，你无需人为连打。
5. **停止条件**（仅以下情况停止轮询）：
   - `task_status` 为 `SUCCESS` 且有 `image_paths` / `absolute_image_paths` → 会话回传；
   - `task_status` 为 `FAILURE` → 报告 `task_id` 与 `task_error`，等用户决定是否重试；
   - 用户明确要求停止或改需求。
6. **工具轮次将尽时**：若仍在排队，向用户说明“任务仍在生成中，请回复继续轮询”，并给出 `task_id`；不要改口称失败。
...
4. 调用 `scripts/generate_image.py`（推荐先 `--no-wait`）。
5. 用同一 `task_id` 轮询至 `SUCCESS` / `FAILURE`。
6. 成功后完成 **会话回传**（见上文）。

## 调用脚本

> Looma Skill 脚本单次最长约 60 秒。高质量/排队出图经常更久，因此 **必须** 异步提交 + 分次查询，且接受总等待可达数分钟。
