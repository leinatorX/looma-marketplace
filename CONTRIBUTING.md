...
3. 完成 `.codex-plugin/plugin.json` 和至少一个 `SKILL.md`。
4. 在 `.agents/plugins/marketplace.json` 末尾追加插件条目。
5. 执行 `python scripts/validate.py --all` 与 `python -m unittest discover -s tests -p "test_*.py"`。
